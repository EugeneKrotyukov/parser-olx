...
def insert_into_parsing_table(table_name, number, title, price, date, time, phone, place, content, bd_name='olx.sqlite3'):
    ...
    try:
        with conn:
            cursor = conn.cursor()
            insert = 'INSERT INTO "{}" (number, title, price, date, time, phone, place, content) VALUES (?, ?, ?, ?, ?, ?, ?, ?), (number, title, price, date, time, phone, place, content)'.format(table_name)
...
def select_from_bd_value(column, row, table_name, bd_name='olx.sqlite3'):
